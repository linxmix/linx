...
Echonest = {
  ...
  identifyTrackMD5: function(md5, options) {
    ...
    $.ajax({
      ...
      success: function(response) {
        var track = Graviton.getProperty(response, 'response.track');
        if (track) {
          options.onSuccess && options.onSuccess(track);
        } else {
          options.onFail && options.onFail(response);
        }
      },
      error: function(xhr) {
        console.error(xhr);
  ...
  fetchTrackProfile: function(track, cb) {
    ...
    Echonest.fetchTrackId(function(track, echonestId) {
