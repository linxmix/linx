Utils = {
  ...
  // TODO: to persist model, add to _collection._realCollection
  // TODO: should i just move all this crap to allow/deny?
  createLocalModel: function(collection, attrs) {
    return collection._localCollection.create(attrs);
  },

  db: function(collection, fnName) {
    var args = Array.prototype.slice.call(arguments, 2);
    var model = collection[fnName].apply(collection, args);

    // if didn't find model, try from localCollection
    console.log("MODEL", model);
    if (model) {
      return model;
    } else if (collection._localCollection) {
      return Utils.db.apply(this, [collection._localCollection, fnName].concat(args));
    } else {
      return;
    }
  },
  ...
  // add localCollection
  defineCollection: function(collectionName, options) {
    var col = Graviton.define(collectionName, options);
    if (options.persist) {
      options.persist = false;
      var localCol = Graviton.define(collectionName + 'Local', options);
      col._localCollection = localCol;
      localCol._realCollection = col;
    }
    return col;
  },
