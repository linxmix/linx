...
Utils = {
  ...
  drawWaveInfo: function() {
    var waves = Waves.all();
    var tracks = waves.map(function(wave) {
      return wave.getTrack();
    });

    function next() {
      Waves.all().forEach(function(wave) { wave.drawBars(); wave.drawSections(); });

    }

    function tracksAnalyzed() {
      return _.all(tracks, function(track) {
    ...
    if (tracksAnalyzed()) {
      ...
      tracks.forEach(function(track) {
        Echonest.fetchTrackAnalysis(track, function() {
          if (tracksAnalyzed()) {
            next();
          }
        });
  ...
  // returns the composition of first and last
  compose: function(firstFn, lastFn) {
    if (firstFn) {
      return function() {
  ...
  // returns the chaining of first and last
  chain: function(firstFn, lastFn) {
    if (firstFn) {
      return function() {
        firstFn.apply(this, arguments);
        return lastFn.apply(this, arguments);
      };
    } else {
      return lastFn;
    }
  ...
  migrateOldLinx: function() {
    ...
    var s3BaseUrl = 'http://s3-us-west-2.amazonaws.com/linx-music';
    ...
    songs.forEach(function(song) {
      var track = Tracks.create({
        ...
        artist: song.artist,
        s3Url: s3BaseUrl + '/songs/' + song._id + "." + song.fileType,
        type: "song",
        flags: [],
    ...
    transitions.forEach(function(transition) {
      ...
      var transitionTrack = Tracks.create({
        ...
        s3Url: s3BaseUrl + '/transitions/' + transition._id + "." + transition.fileType,
        type: "transition",
        flags: [],
